	...
	public function restaurant_menu_render_callback( array $block_attributes, string $content, WP_Block $block ): string {
		...
		if (file_exists($file_path)) {
			try {
				...
				// Cattura l'output del file di rendering
				ob_start();
				$return_value = include $file_path;
				$output = ob_get_clean();
				
				// Se output è vuoto, ma il file ha restituito un valore, usa quel valore
				if (empty($output) && $return_value !== 1 && !empty($return_value)) {
					return $return_value;
				}
				
				// Se output è ancora vuoto, mostra un messaggio di errore
				if (empty($output)) {
